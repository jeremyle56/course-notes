...
\defn{Associates, Irreducibles and Primes}{
    \begin{itemize}
        \item Elements \am and \bm of an integral domain \Rm are called associates if \(a = ub\), for some unit \(u\) of \Rm.
        ...
        \item A non-zero, non-unit element \(\rho\) of the integral domain \Rm is said to be prime if it has the property
...
\thrm{Greatest Common Divisor}{
    Let \(a, b\) be integers, not both zero. Then \am positive intger \(g\) is the greatest common divisor of \am and \bm if and only if \(g\) is a common divisor and every common divisor is a factor of \(g\).
}

\defn{GCD in Rings}{
    Let \(a, b\) be elements in a commutative ring \Rm. An element \(g \in R\) is a greatest common divisor of \am and \bm in \Rm if \(g \mid a, g \mid b\) and every common divisor of \am and \bm is a factor of \(g\).
}

\subsection{Ideals}
\defn{Ideal}{
    Let \Rm be a commutative ring with identity. A subset \(I\) of \Rm is called an ideal of \Rm if it has the following three proprties:
    \begin{itemize}
        \item 0 is in \(I\).
...
\prop{Smallest Ideal}{
    Let \Rm be a commutative ring with identity, and \(\{a_1, \dots, a_n\} \subset R\). Then the set
    \[\{r_1a_1 + \cdots + r_na_n: r_1, \dots, r_n \in R\}\]
    is the smallest ideal of \Rm containing \(\{a_1, \dots, a_n\}\).
}

\defn{Pinrcipal Ideal}{
    An ideal \(I\) of a ring \Rm is said to be principal if there exists \(a \in R\) such that \(I = \langle a \rangle  = \{ax : x \in R\}\).
}
...
\defn{Big-Oh and Little-Oh Notations}{
    ...
    \begin{itemize}
        ...
        \item \(f(x) = o(g(x))\) if \(\lim_{x \to \infty}\abs*{f(x)}/g(x) = 0\) or, alternatively, iff for any \(\epsilon > 0\) we have \(\abs*{f(x)} \leq \epsilon g(x)\) for all sufficiently large \(x\).
    \end{itemize}
}
