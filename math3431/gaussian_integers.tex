...
\prop{Solutions if \(p\) is a Sum of Two Squares}{
...
\defn{Waring Problem}{
    Is it true for any integer \(k \geq 1\) there is a number \(g(k)\) such that every positive integer is the sum of \(g(k)\) integer k-th powers?
}

\thrm{David Hilbert}{
    For any integer \(k \geq 1\) the number \(g(k)\) exists.
}