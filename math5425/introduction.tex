...
Given \(x, y \in V\), let \(d_G(x, y)\) be the length of a shortest path from \(x\) to \(y\) in \(G\), called the \textbf{distance} from \(x\) to \(y\) in \(G\). Set \(d_G(x, y) = \inf\) if no such path exists. \\
...
\begin{proposition}
    Every graph \(G\) with contains a cycle satisfies \(g(G) \leq 2 \diam(G) + 1\).
\end{proposition}
